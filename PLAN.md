...
| 3.2 | 可拖动分隔条 | 浅灰色分隔条，拖拽调整两区域高度，最小 150px，最大 350px | §4 |
| 3.3 | 名称哈希 → 颜色映射 | `H = hash(name) % 360, S=0.6, V=0.9` | §6.1 |
| 3.4 | 火焰图递归渲染 | 使用 `ImDrawList` 的 `AddRectFilled` / `AddText`，按深度、名称字母序排列子节点 | §6.1 |
| 3.5 | self cost 可视化 | 在色块右侧预留 self 段，亮度 +20% | §6.2 |
| 3.6 | 最小宽度裁剪 | 宽度 < 1px 的色块不绘制 | §6.1, §8 |
...
| # | 任务 | 说明 | 预期收益 |
|---|------|------|----------|
| 8.1 | 引入高精度曲线永久缓存 | 在 `TimelineView` 中使用 `std::unordered_map` 缓存已计算的高精度曲线，避免来回 Hover 时的重复计算。 | 消除重复 Hover 时的卡顿，实现瞬间出图 |
| 8.2 | 渐进式渲染 (LOD) 架构 | 鼠标 Hover 新节点时，主线程瞬间计算并绘制 100 个点的低精度曲线，保证 0 延迟视觉反馈。 | 彻底消除首次 Hover 时的“拖拽泥泞感”和掉帧 |
| 8.3 | 异步高精度计算与中断机制 | 启动后台线程计算高精度曲线。若鼠标移走，立即发送 `cancelFlag` 中断计算；若计算完成，无缝替换低精度曲线并存入缓存。 | 保证高精度数据的最终一致性，且不浪费 CPU 算力 |